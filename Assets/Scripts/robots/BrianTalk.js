...
public var brianChat: Sprite[];
 ...
 private var timBubble: GameObject;
...
function Start () {
	...
	SoundManager.instance.changeRobotVol(.3);
	...
	timBubble = GameObject.Find("timBubble");
...
function Update () {
	...
	if(panToGirl && Time.time < nextTime){
		...
		if(done){ 
			...
			if(!laserOn){
				GameManager.instance.GetComponent(GamePlay).girlScript.enabled = true;
